...
import { z } from 'zod';
import { supabase, Room, RoomInsert, Participant, ParticipantInsert } from '../../lib/supabase';
import { authenticateUser, optionalAuth, AuthenticatedRequest, APIError } from '../middleware/auth';
...
router.get('/', authenticateUser, async (req: AuthenticatedRequest, res) => {
	try {
		...
		if (role && typeof role === 'string') {
			query = query.eq('participants.role', role as 'owner' | 'editor' | 'viewer');
		}
		...
		if (error) {
			throw new APIError('Failed to fetch rooms', 500, 'DATABASE_ERROR');
		}
...
router.post('/', authenticateUser, async (req: AuthenticatedRequest, res) => {
	try {
		...
		if (participantError) {
			...
			throw new APIError('Failed to create room participant', 500, 'DATABASE_ERROR');
		}
...
router.get('/:roomId', authenticateUser, async (req: AuthenticatedRequest, res) => {
	try {
...
router.put('/:roomId', authenticateUser, async (req: AuthenticatedRequest, res) => {
	try {
		const { roomId } = req.params;
		const userId = req.user!.id;
...
router.delete('/:roomId', authenticateUser, async (req: AuthenticatedRequest, res) => {
	try {
		const { roomId } = req.params;
		const userId = req.user!.id;
...
router.post('/:roomId/join', authenticateUser, async (req: AuthenticatedRequest, res) => {
	try {
		const { roomId } = req.params;
		const userId = req.user!.id;
		...
		if (roomError || !room) {
			throw new APIError('Room not found or inactive', 404, 'ROOM_NOT_FOUND');
		}
		...
		if (existingParticipant) {
			throw new APIError('Already a participant in this room', 409, 'ALREADY_PARTICIPANT');
		}
		...
		if (participantCount && participantCount >= room.max_participants) {
			throw new APIError('Room is full', 409, 'ROOM_FULL');
		}
...
router.post('/:roomId/leave', authenticateUser, async (req: AuthenticatedRequest, res) => {
	try {
		const { roomId } = req.params;
		const userId = req.user!.id;
		...
		if (participant.role === 'owner') {
			const { count: otherParticipants } = await supabase
				.from('participants')
				.select('id', { count: 'exact' })
				.eq('room_id', roomId)
				.neq('user_id', userId);
			...
			if (otherParticipants && otherParticipants > 0) {
				throw new APIError('Room owner cannot leave while other participants are present. Transfer ownership or delete the room.', 409, 'OWNER_CANNOT_LEAVE');
			}
		}
