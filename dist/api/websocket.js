...
const joinRoomEventSchema = zod_1.z.object({
    roomId: zod_1.z.string().uuid(),
    token: zod_1.z.string(),
});
const cursorUpdateEventSchema = zod_1.z.object({
    roomId: zod_1.z.string().uuid(),
...
const operationEventSchema = zod_1.z.object({
    roomId: zod_1.z.string().uuid(),
    documentId: zod_1.z.string().uuid(),
...
function checkRateLimit(socketId, limit = 100, windowMs = 60000) {
    const now = Date.now();
        ...
        io.on('connection', (socket) => {
            ...
            socket.on('join-room', async (data) => {
                try {
                    ...
                    const roomSubscription = (0, supabase_1.createRoomSubscription)(validatedData.roomId, (payload) => {
                        socket.to(validatedData.roomId).emit('room-update', payload);
                    });
                    const presenceSubscription = (0, supabase_1.createPresenceSubscription)(validatedData.roomId, (payload) => {
            ...
            socket.on('document-operation', async (data) => {
                try {
                    ...
                    const { data: document } = await supabase_1.supabase
                        .from('documents')
                    ...
                    for (const operation of validatedData.operations) {
                        ...
                        // Store operation
                        const { data: storedOperation } = await supabase_1.supabase
                            .from('operations')
                            .insert({
                            document_id: validatedData.documentId,
                            participant_id: userInfo.participantId,
