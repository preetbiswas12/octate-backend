...
import { supabase, supabaseAdmin, Room, RoomInsert } from '../../lib/supabase';
import { z } from 'zod';
...
function checkRateLimit(ip: string, limit: number = 100, windowMs: number = 15 * 60 * 1000) {
	const now = Date.now();
...
async function handleGetRoom(req: VercelRequest, res: VercelResponse) {
	const { id } = req.query;
	...
	const hasAccess = room.participants?.some((p: any) => p.user_id === user.id) || room.owner_id === user.id;
	if (!hasAccess) {
...
async function handleDeleteRoom(req: VercelRequest, res: VercelResponse) {
	const { id } = req.query;
