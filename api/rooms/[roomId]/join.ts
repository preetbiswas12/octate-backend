...
import { supabase, ParticipantInsert } from '../../../lib/supabase';
import { z } from 'zod';
...
async function handleJoinRoom(req: VercelRequest, res: VercelResponse, roomId: string) {
...
async function handleLeaveRoom(req: VercelRequest, res: VercelResponse, roomId: string) {
	const user = await getAuthenticatedUser(req);
