...
import { supabase, DocumentInsert } from '../../../lib/supabase';
import { z } from 'zod';
...
async function handleGetDocuments(req: VercelRequest, res: VercelResponse, roomId: string) {
...
async function handleCreateDocument(req: VercelRequest, res: VercelResponse, roomId: string) {
	const user = await getAuthenticatedUser(req);
...
async function handleUpdateDocument(req: VercelRequest, res: VercelResponse, roomId: string) {
	const user = await getAuthenticatedUser(req);
	const participant = await checkRoomAccess(user.id, roomId);
...
async function handleDeleteDocument(req: VercelRequest, res: VercelResponse, roomId: string) {
	const user = await getAuthenticatedUser(req);
	const participant = await checkRoomAccess(user.id, roomId);
